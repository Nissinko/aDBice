...
<!-- {{$php_json = json_encode(config('job_class'))}} -->
            ...
            for (let val of js_array[key]){
                $(document).on('click','#middle' + i + ' > tr:nth-child(' + (j+1) + ')' , function(){
                    ...
                    if(parantExistanceFlag) {
                        if (param == 0){
                            window.opener.document.getElementById('job_class').value = key + " > " + val;
                        }
                        else{
                            window.opener.document.getElementById('hope_job_class').value = key + " > " + val;
                        }
                        window.close();
                    }
                });
...
<body>
<h2>職種を選択</h2>
<div id = "big_area" class="row_area">
    <table>
        <tbody id="big_table">
        @foreach(array_keys(config('job_class')) as $i => $key)
...
<div id="middle_area" class="row_area">
    @foreach(array_keys(config('job_class')) as $i => $key)
        <table>
            <tbody id={{"middle".$i}} style="display:none;">
            @foreach(config('job_class')[$key] as $j => $val)
                <tr class="clickable">
                    <td>{{$val}}</td>
                </tr>
            @endforeach
